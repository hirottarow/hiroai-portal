...
- 株数と継続保有月数の条件を満たす段階のうち **`minShares` 最大**を採る（同じなら `minHoldMonths`、それも同じなら `amount`）。
  2026-09-10 の v1.15.03.046 で「amount 最大」から変更した。**段階表はもともと株数の区分表なので、区分は株数で決める。**
  amount 最大だと、区分が上がると額が下がる優待（相鉄HD 9003・阪急阪神HD 9042）で実際より多い残高が入ってしまう。
  **登録画面で保有開始年月が未入力なら `minHoldMonths` が 0 より大きい段階は採用しない**（参考表示にとどめる）。
...
**辞書には「毎年あるのか」を表す軸が無く、既定は「毎年ある」に倒れる。**
一回限りの記念優待をそのまま書くと、権利月・発送月を過ぎた瞬間から翌年の予定を出し続ける
（`QuoCalculator.nextOccurrenceMonth` も `ArrivalRules` も年を循環する作り）。これを止めるのが `validUntil`。

- 形式は **`"YYYY-MM"`（月粒度）。その月の末日まで有効で、翌月1日から「終了」扱い。**
- 置ける場所は2つ:
  - **エントリ直下** … そのエントリ全体が一回限り（例: Green Earth Institute 9212 に `"validUntil": "2026-12"`）
  - **`tiers[]` / `quo.tiers[]` の各段** … 段だけが期間限定（**恒常優待への今年だけの記念上乗せ**）
- 空文字・省略は「期限なし＝ずっと有効」。**形式違いも「期限なし」に倒す**
  （外れた推定で優待を消すより、黙って従来どおり出す方が害が小さい）。
- `validFrom` は無い。「来年から始まる優待」を先に書く運用が無いので足していない。

**期限が過ぎるとアプリがどうするか**（v1.15.03.049〜）:

| 場所 | 期限内 | 期限後 |
|---|---|---|
| QUOタブの「次の権利確定 / 次の到着目安」 | 出す | **出さない** |
| ホームの「そろそろ届くはず」 | 出す | **出さない**（`shipMonthsByKey` に入れない） |
| 段（tiers）の採用・再登録時の残高 | 入る | **入らない**（段階表からも消える） |
| 辞書一覧・QUOタブの並び | 従来どおり | **最後へ回し「終了」バッジを付ける** |
...
**`siteOnly` の銘柄は「そろそろ届くはず」に乗らない。**
`TemplateRepository.applyCatalog` が `shipMonthsByKey` を **`templates` からしか作らない**ため
（2026-09-19 にコードで確認）。記念優待を `siteOnly` に置いている間はこれが安全弁として効くが、
**`templates` に移した瞬間に効かなくなる**ので、一回限りの優待には必ず `validUntil` を書くこと。
